/*
    -- MAGMA (version 2.0) --
       ...
       @generated from magmablas/zgetf2_kernels.cu, normal z -> s, Sat Mar 27 20:31:36 2021
*/
                   ...
                   int step, int lda, magma_int_t** ipiv_array, magma_int_t *info_array, int gbstep)
{
   ...
   // const int batchid = blockIdx.z;
    ...
    float *x_start = x_array[batchid] + xj * lda + xi;
                     ...
                     magma_int_t gbstep, magma_queue_t queue)
{
    ...
    if( length <= 15360 ) {
        ...
        hipblasIsamax(queue->hipblas_handle(), length, (const float*)(x + step * lda + step), 1, (int*)(ipiv+step));
        magma_spivcast<<< 1, 1, 0, queue->cuda_stream() >>>( ipiv+step );
                                      ...
                                      magma_int_t batchCount, magma_queue_t queue)
{
    ...
    */
    ...
    for(magma_int_t i = 0; i < batchCount; i+=max_batchCount) {
        ...
        switch(n){
            case  1: sscal_sger_1d_kernel_batched< 1><<<grid, threads, 0, queue->cuda_stream()>>>( m, step, dA_array+i, ai, aj, lda, info_array+i, gbstep);break;
            case  2: sscal_sger_1d_kernel_batched< 2><<<grid, threads, 0, queue->cuda_stream()>>>( m, step, dA_array+i, ai, aj, lda, info_array+i, gbstep);break;
            case  3: sscal_sger_1d_kernel_batched< 3><<<grid, threads, 0, queue->cuda_stream()>>>( m, step, dA_array+i, ai, aj, lda, info_array+i, gbstep);break;
            case  4: sscal_sger_1d_kernel_batched< 4><<<grid, threads, 0, queue->cuda_stream()>>>( m, step, dA_array+i, ai, aj, lda, info_array+i, gbstep);break;
            case  5: sscal_sger_1d_kernel_batched< 5><<<grid, threads, 0, queue->cuda_stream()>>>( m, step, dA_array+i, ai, aj, lda, info_array+i, gbstep);break;
            case  6: sscal_sger_1d_kernel_batched< 6><<<grid, threads, 0, queue->cuda_stream()>>>( m, step, dA_array+i, ai, aj, lda, info_array+i, gbstep);break;
            case  7: sscal_sger_1d_kernel_batched< 7><<<grid, threads, 0, queue->cuda_stream()>>>( m, step, dA_array+i, ai, aj, lda, info_array+i, gbstep);break;
            case  8: sscal_sger_1d_kernel_batched< 8><<<grid, threads, 0, queue->cuda_stream()>>>( m, step, dA_array+i, ai, aj, lda, info_array+i, gbstep);break;
            default: sscal_sger_1d_generic_kernel_batched<<<grid, threads, 0, queue->cuda_stream()>>>(m, n, step, dA_array+i, ai, aj, lda, info_array+i, gbstep);
        }
    }
    return 0;
}
